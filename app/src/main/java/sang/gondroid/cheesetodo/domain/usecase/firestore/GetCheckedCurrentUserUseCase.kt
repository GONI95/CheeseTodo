...
class GetCheckedCurrentUserUseCase(private val reviewTodoRepository: ReviewTodoRepository, private val ioDispatcher: CoroutineDispatcher) {
    ...
    suspend operator fun invoke(model: ReviewTodoModel) : JobState = withContext(ioDispatcher) {
        LogUtil.d(Constants.TAG, "GetCheckedCurrentUserUseCase getCheckedCurrentUser()")
        return@withContext reviewTodoRepository.getCheckedCurrentUser(model)
    }
}